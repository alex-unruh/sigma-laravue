  ...
  <ul :class="containerClass" :role="root ? 'menubar' : 'menu'">
    <template v-for="(item, i) of model" :key="item.label + i.toString()">
      <li role="none" :class="getItemClass(item)" :style="item.style" v-if="visible(item) && !item.separator" @mouseenter="onItemMouseEnter($event, item)">
        <inertia-link v-if="item.to && !item.disabled" :href="item.to" custom v-slot="{ navigate, href }">
        ...
        <sub-menu :model="item.items" v-if="visible(item) && item.items" :key="item.label + '_sub_'" :mobileActive="mobileActive" @leaf-click="onLeafClick" @keydown-item="onChildItemKeyDown" :parentActive="item === activeItem" />
      </li>
