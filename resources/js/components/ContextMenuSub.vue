    ...
    <ul ref="container" :class="containerClass" role="menu" v-if="root ? true : parentActive">
      <template v-for="(item, i) of model" :key="i">
        <li role="none" :class="getItemClass(item)" :style="item.style" v-if="visible(item) && !item.separator" @mouseenter="onItemMouseEnter($event, item)">
          <inertia-link v-if="item.to && !item.disabled" :href="route(item.to)" custom @click="onItemClick($event, item, navigate)" :class="getLinkClass(item)" v-ripple>
            <span :class="['p-menuitem-icon', item.icon]"></span>
            <span class="p-menuitem-text">{{ item.label }}</span>
          </inertia-link>
          <a v-else :href="item.url" :class="getLinkClass(item)" :target="item.target" @click="onItemClick($event, item)" v-ripple :aria-haspopup="item.items != null" :aria-expanded="item === activeItem" role="menuitem" :tabindex="item.disabled ? null : '0'">
            <span :class="['p-menuitem-icon', item.icon]"></span>
            <span class="p-menuitem-text">{{ item.label }}</span>
            <span class="p-submenu-icon pi pi-angle-right" v-if="item.items"></span>
          </a>
          <sub-menu :model="item.items" v-if="visible(item) && item.items" :key="i" @leaf-click="onLeafClick" :parentActive="item === activeItem" />
        </li>
