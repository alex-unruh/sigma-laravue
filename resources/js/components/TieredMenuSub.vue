  ...
  <ul ref="element" :class="containerClass" role="'menubar' : 'menu'" aria-orientation="horizontal">
    <template v-for="(item, i) of model" :key="item.label + i.toString()">
      <li :class="getItemClass(item)" :style="item.style" v-if="visible(item) && !item.separator" @mouseenter="onItemMouseEnter($event, item)" role="none">
        <inertia-link @click="onItemClick($event, item, navigate)" :class="getLinkClass(item)" @keydown="onItemKeyDown($event, item)" v-if="item.to && !item.disabled" :href="route(item.to)" v-ripple custom>
          <span :class="['p-menuitem-icon', item.icon]"></span>
          <span class="p-menuitem-text">{{ item.label }}</span>
        </inertia-link>
        <a v-else :href="item.url" :class="getLinkClass(item)" :target="item.target" :aria-haspopup="item.items != null" :aria-expanded="item === activeItem" @click="onItemClick($event, item)" @keydown="onItemKeyDown($event, item)" role="menuitem" :tabindex="item.disabled ? null : '0'" v-ripple>
          <span :class="['p-menuitem-icon', item.icon]"></span>
          <span class="p-menuitem-text">{{ item.label }}</span>
          <span class="p-submenu-icon pi pi-angle-right" v-if="item.items"></span>
        </a>
        <sub-menu :model="item.items" v-if="visible(item) && item.items" :key="item.label + '_sub_'" @leaf-click="onLeafClick" @keydown-item="onChildItemKeyDown" :parentActive="item === activeItem" />
      </li>
