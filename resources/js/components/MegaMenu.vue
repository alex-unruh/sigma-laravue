    ...
    <ul class="p-megamenu-root-list" role="menubar">
      <template v-for="(category, index) of model" :key="category.label + '_' + index">
        <li v-if="visible(category)" :class="getCategoryClass(category)" :style="category.style" @mouseenter="onCategoryMouseEnter($event, category)" role="none">
          <inertia-link v-if="category.to && !category.disabled" :href="route(category.to)" custom :class="getLinkClass(category)" @click="onCategoryClick($event, category, navigate)" @keydown="onCategoryKeydown($event, category)" v-ripple>
            <span v-if="category.icon" :class="getCategoryIcon(category)"></span>
            <span class="p-menuitem-text">{{ category.label }}</span>
          </inertia-link>
          <a v-else :href="category.url" :class="getLinkClass(category)" :target="category.target" @click="onCategoryClick($event, category)" @keydown="onCategoryKeydown($event, category)" role="menuitem" :aria-haspopup="category.items != null" :aria-expanded="category === activeItem" :tabindex="category.disabled ? null : '0'" v-ripple>
            <span v-if="category.icon" :class="getCategoryIcon(category)"></span>
            <span class="p-menuitem-text">{{ category.label }}</span>
                ...
                <ul v-for="(submenu, submenuIndex) of column" class="p-megamenu-submenu" :key="submenu.label + '_submenu_' + submenuIndex" role="menu">
                  ...
                  <template v-for="(item, i) of submenu.items" :key="item.label + i.toString()">
                    <li role="none" :class="getSubmenuItemClass(item)" :style="item.style" v-if="visible(item) && !item.separator">
                      <inertia-link v-if="item.to && !item.disabled" :href="route(item.to)" custom :class="getLinkClass(item)" @click="onLeafClick($event, item, navigate)" role="menuitem" v-ripple>
